    ...
    if (payloadCommand === 'explore_attack') {
        ...
        */
        ...
        if (text === 'путешествие' || payloadCommand === 'travel' || payloadCommand?.startsWith('travel_loc_page_')) {
          ...
          kb.textButton({ label: (char.rpg?.activeQuest ? '🔴 Мое задание' : '📋 Задания'), payload: { command: 'guild_my_quest' }, color: (char.rpg?.activeQuest ? Keyboard.NEGATIVE_COLOR : Keyboard.SECONDARY_COLOR) });
          kb.row();
        ...
        if (payloadCommand === 'bs_enhance') {
          ...
          if (success) {
             ...
             if (enhance > 5) {
                 ...
                 const newItemId = buildItemId(baseId, dropLevel, slots, gems, level);
                 char.rpg.equipment[slot] = newItemId;
