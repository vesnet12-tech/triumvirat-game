                  ...
                  <div key={char.id} className="bg-neutral-950 border border-neutral-800 rounded-lg p-5">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex gap-2">
                        <button 
                          onClick={() => {
                            const amount = prompt('Введите количество золота для ' + char.name + ':');
                            if (amount && !isNaN(Number(amount))) {
                              fetch('/api/god/give-money', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ charId: char.id, amount })
                              }).then(() => { alert('Золото выдано!'); window.location.reload(); });
                            }
                          }}
                          className="flex items-center gap-1 bg-amber-500/20 text-amber-400 px-3 py-1.5 rounded hover:bg-amber-500/30 transition-colors text-sm"
                        >
                          💰 Дать золото
                        </button>
                        <button 
                          onClick={() => {
                            if (confirm('Воскресить ' + char.name + '?')) {
                              fetch('/api/god/resurrect', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ charId: char.id })
                              }).then(() => { alert('Персонаж воскрешен!'); window.location.reload(); });
                            }
                          }}
                          className="flex items-center gap-1 bg-emerald-500/20 text-emerald-400 px-3 py-1.5 rounded hover:bg-emerald-500/30 transition-colors text-sm"
                        >
                          ✨ Воскресить
                        </button>
                        <button 
