...
async function initBot() {
  ...
  if (process.env.GEMINI_API_KEY && !loadedKeys.includes(process.env.GEMINI_API_KEY)) {
          ...
          if (!guildBoard.quests || guildBoard.quests.length === 0 || (nowMs - guildBoard.lastRefresh > limitMs)) {
            ...
            for (let i=0; i<3; i++) {
              ...
              if (isGather) {
                  ...
                  let matInfo = Object.values(ITEM_CATALOG).find(i => i.id === matId) || ITEM_CATALOG[matId] || { name: 'Материал' };
        ...
        if (char.actionEndTime && char.actionEndTime <= now && !char.actionNotified) {
          ...
          if (char.actionType === 'work') {
            ...
            if (r < 0.3) {
                ...
                char.rpg.inventory = char.rpg.inventory || [];
                char.rpg.inventory.push({ itemId: pItem.id, amount: 1 });
...
app.post('/api/god/give-money', async (req, res) => {
  try {
    ...
    char.gold = (char.gold || 0) + parseInt(amount);
    await safeSetDoc(doc(db, 'characters', charId), { gold: char.gold }, { merge: true });
    
    if (vk && char.ownerId) {
      await vk.api.messages.send({
        peer_id: char.ownerId,
        random_id: Math.floor(Math.random() * 1000000),
        message: `⚡ ГЛАС БОЖИЙ ⚡\nБог решил дать вам ${amount} 💰! Теперь у вас ${char.gold} 💰.`
      });
    }
    res.json({ success: true });
  } catch (e) {
...
app.post('/api/god/resurrect', async (req, res) => {
  try {
    ...
    // reset death 
    await safeSetDoc(doc(db, 'characters', charId), { 
      'rpg.baseStats.hp': char.rpg?.baseStats?.maxHp || 10,
      'rpg.deadUntil': null,
      'rpg.combatId': null
    }, { merge: true });
    ...
    if (vk && char.ownerId) {
      await vk.api.messages.send({
        peer_id: char.ownerId,
        random_id: Math.floor(Math.random() * 1000000),
        message: `⚡ ГЛАС БОЖИЙ ⚡\nБоги сжалились над вами! Вы были воскрешены немедленно.`
      });
    }
    res.json({ success: true });
  } catch (e) {
