      ...
      if (itemDef) {
        if (itemDef.stats) {
          ...
          if (itemDef.type === 'accessory') {
             ...
             // Криты скалируются от уровня заточки для аксессуаров
             if (itemDef.stats.critDamage) {
                if (itemDef.rarity === 'rare') stats.critDamage += enhance * 1;
                else if (itemDef.rarity === 'epic') stats.critDamage += enhance * 2;
                else if (itemDef.rarity === 'legendary') stats.critDamage += enhance * 3;
             }
             if (itemDef.stats.critRate) {
                if (itemDef.rarity === 'rare') stats.critRate += enhance * 0.5;
                else if (itemDef.rarity === 'epic') stats.critRate += enhance * 1;
                else if (itemDef.rarity === 'legendary') stats.critRate += enhance * 2;
             }
          }
          if (itemDef.setName) {
