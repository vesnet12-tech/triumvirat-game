...
async function renderCombatUI(context: any, char: any, extraLog = '') {
  ...
  const keyboard = Keyboard.builder()
    .textButton({ label: '⚔️ Атака', payload: { command: 'combat', combatAction: 'attack' }, color: Keyboard.NEGATIVE_COLOR })
    .textButton({ label: '🛡️ Защита', payload: { command: 'combat', combatAction: 'defend' }, color: Keyboard.PRIMARY_COLOR })
    .row()
    .textButton({ label: '✨ Навыки', payload: { command: 'combat_skills_menu' }, color: Keyboard.POSITIVE_COLOR })
    .textButton({ label: '🏃 Побег', payload: { command: 'combat', combatAction: 'flee' }, color: Keyboard.SECONDARY_COLOR })
    ;
...
function generateShopItems(cat: string, count: number, playerLevel: number = 1) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (pool.length > 0) {
      ...
      if (['weapon', 'armor', 'helmet', 'shield', 'accessory'].includes(randomItem.type)) {
        let levelOffset = Math.floor(Math.random() * 6); // 0 to 5
...
// ... (inside message handler, after PvP commands)
    ...
    if (isLeaveCommand || isMainMenuText || isMainMenuCommand) {
      try {
        ...
        if (payloadCommand === 'port_future') {
          const kb = Keyboard.builder()
            .textButton({ label: '⬅️ Назад', payload: { command: 'city_explore', quarter: 'port' }, color: Keyboard.SECONDARY_COLOR });
          await context.send({ message: `В будущем обновлении здесь появится перемещение в океане и морские путешествия!`, keyboard: kb });
          return;
        }
        ...
        if (payloadCommand === 'house_eat') {
           if (!char.rpg.foodBuff) {
              ...
              const kb = Keyboard.builder().textButton({ label: '🔙 Назад', payload: { command: 'house_menu' }, color: Keyboard.PRIMARY_COLOR });
              await context.send({ message: `❌ Вы уже сыты. Бафф действует еще на ${char.rpg.foodBuff.charges} боев.`, keyboard: kb });
           }
           return;
        }
        ...
        if (payloadCommand === 'tavern_sleep') {
          ...
          if (!char.rpg.baseStats) {
            char.rpg.baseStats = { hp: maxHp, mp: maxMp, maxHp, maxMp, attack: 5, magicAttack: 0, defense: 2, magicDefense: 0, agility: 5, critRate: 5, critDamage: 150 };
          }
    ...
    if (payloadCommand === 'explore_attack') {
        ...
        if (payloadCommand === 'guild_talk' || (char.location === 'city' && text.includes('регистратор'))) {
          ...
          await context.send({ message: `👤 Регистратор: "Приветствую! Пока что мы переоформляем бумаги, так что полноценных заданий нет. Но вы всегда можете пойти поохотиться в лес самостоятельно!"`, keyboard: kb });
          return;
        }
        ...
        */
        ...
        if (payloadCommand === 'shop_cat') {
          ...
          if (!shopData[cat] || shopData[cat].length === 0 || (now.getTime() - lastRefresh.getTime() > threeHours)) {
            shopData[cat] = generateShopItems(cat, 20, char.rpg?.level || 1);
        ...
        if (payloadCommand === 'bs_enhance') {
          ...
          let msg = '';
          if (success) {
             const newItemId = buildItemId(baseId, enhance + 1, slots, gems);
             char.rpg.equipment[slot] = newItemId;
             msg = `✨ Успех! За ${cost} 💰 твой ${item.name} заточен до +${enhance + 1}!`;
          } else {
             msg = `💥 Провал! Камень и ${cost} 💰 уничтожены.`;
             if (enhance > 5) {
                 const dropLevel = Math.max(0, enhance - 1);
                 const newItemId = buildItemId(baseId, dropLevel, slots, gems);
                 char.rpg.equipment[slot] = newItemId;
                 msg += ` Уровень заточки снизился до +${dropLevel}.`;
             }
          }
        ...
        if (text === 'снаряжение' || payloadCommand === 'equipment' || payloadCommand?.startsWith('eq_cat_') || payloadCommand?.startsWith('eq_page_')) {
          ...
          if (category === 'accessory') slotsToCheck.push('accessory');
        ...
        if (payloadCommand === 'gem_insert') {
           ...
           // Create new item ID
           gems.push(gem.id);
           const newItemId = buildItemId(baseId, enhance, slots, gems);
        ...
        if (text === 'скиллы' || payloadCommand === 'skills') {
          ...
          let msg = `✨ Ваши навыки:\nАктивные: ${equippedActive.length}/6 | Пассивные: ${equippedPassive.length}/6\n\nВыберите категорию:`;
          const keyboard = Keyboard.builder()
            .textButton({ label: '🔥 Активные', payload: { command: 'skills_cat_active' }, color: Keyboard.PRIMARY_COLOR })
            .row()
            .textButton({ label: '🛡️ Пассивные', payload: { command: 'skills_cat_passive' }, color: Keyboard.PRIMARY_COLOR })
            .row()
