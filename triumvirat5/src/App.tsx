  ...
  const renderDatabaseContent = () => {
    const computeItemPreview = (item: any, level: number) => {
      ...
      if (sumLinear > 0) {
        for (let k in cloned.stats) {
            if (!PERCENTAGE_STATS.includes(k) && typeof cloned.stats[k] === 'number') {
                const originalBase = (item.stats as any)[k] as number;
                ...
                cloned.stats[k] = Math.floor(newVal);
            }
        }
      }
      ...
      if (cloned.type === 'weapon') {
          if (cloned.rarity === 'common' || cloned.rarity === 'uncommon') {
              delete cloned.stats.critRate;
              delete cloned.stats.critDamage;
          } else if (cloned.rarity === 'rare') {
              cloned.stats.critRate = 1;
              cloned.stats.critDamage = 5;
          } else if (cloned.rarity === 'epic') {
              cloned.stats.critRate = 2 + Math.floor(level / 10) * 2;
              cloned.stats.critDamage = 5 + Math.floor(level / 10) * 5;
          } else if (cloned.rarity === 'legendary') {
              cloned.stats.critRate = 5 + Math.floor(level / 10) * 2;
              cloned.stats.critDamage = 15 + Math.floor(level / 10) * 5;
          }
      } else {
          if (cloned.rarity === 'common' || cloned.rarity === 'uncommon') {
              delete cloned.stats.critRate;
