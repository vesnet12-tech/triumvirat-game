...
async function initBot() {
  ...
  vk.updates.on('message_new', async (context) => {
    ...
    const uniqueMsgId = context.id || context.conversationMessageId || context.createdAt;
    if (uniqueMsgId !== undefined) {
       ...
       try {
           const msgDocRef = doc(db, 'settings', 'msg_' + msgIdKey.toString());
           const isGlobalDup = await runTransaction(db, async (t) => {
               const docSnap = await t.get(msgDocRef);
               if (docSnap.exists()) {
                   return true;
               }
               t.set(msgDocRef, { processedAt: Date.now() });
               return false;
           });
           
           if (isGlobalDup) {
               console.log(`[Global Deduplication] Message ${msgIdKey} already processed globally. Skipping.`);
               return;
           }
       } catch(e) {
           console.error("Global deduplication error", e);
           // If dedup itself fails due to DB, we have a hard choice.
           // To ensure we don't spam duplicate messages during a cloud run instance burst,
           // we MUST rely on the memory set. If memory doesn't have it, we might duplicate, 
           // but normally memory is enough for a single instance.
       }
    }
    ...
    // Global User Lock via Firestore
    let hasGlobalLock = false;
    try {
        const lockRef = doc(db, 'users', senderId.toString() + '_lock');
        hasGlobalLock = await runTransaction(db, async (t) => {
            const snap = await t.get(lockRef);
            if (snap.exists()) {
                if (Date.now() - snap.data().lockedAt < 60000) {
                    return false; // Still globally locked
                }
            }
        ...
        if (!hasGlobalLock) {
            console.log(`[Global Lock] User ${senderId} is already globally locked. Skipping.`);
            userLocks.delete(senderId);
            return;
        }
    } catch(e) {
...
Текущий сюжет: ${currentPlot}${charInfo}`;
      ...
      if (hasGlobalLock) {
         try {
             await deleteDoc(doc(db, 'users', senderId.toString() + '_lock'));
         } catch(e) {}
      }
    }
  });
