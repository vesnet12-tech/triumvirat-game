...
export function calculateTotalStats(rpg: CharacterRPGData) {
  ...
  for (const slot of Object.values(rpg.equipment)) {
    if (slot) {
      const { enhance, gems } = parseItemId(slot);
      const itemDef = getItem(slot);
      if (itemDef) {
        if (itemDef.stats) {
          ...
          stats.attack += (itemDef.stats.attack || 0);
          stats.magicAttack = (stats.magicAttack || 0) + (itemDef.stats.magicAttack || 0);
          stats.defense += (itemDef.stats.defense || 0);
          stats.magicDefense = (stats.magicDefense || 0) + (itemDef.stats.magicDefense || 0);
          stats.agility += (itemDef.stats.agility || 0);
...
export function equipItem(rpg: CharacterRPGData, itemId: string, characterClass?: string): { success: boolean; message: string } {
  ...
  let targetSlot = item.type as keyof typeof rpg.equipment;
  if (item.type === 'accessory') {
     if (!rpg.equipment.accessory) targetSlot = 'accessory';
     else if (!rpg.equipment.accessory2) targetSlot = 'accessory2';
     else targetSlot = 'accessory'; // Replace the first one if both are full
  }

  // If already equipped something in that slot, unequip it first (move to inventory)
  const currentEquipped = rpg.equipment[targetSlot];
  if (currentEquipped) {
  ...
  // Equip
  rpg.equipment[targetSlot] = itemId;
...
export function unequipItem(rpg: CharacterRPGData, slotOrName: string): { success: boolean; message: string } {
  ...
  const slotMap: Record<string, keyof CharacterRPGData['equipment']> = {
    'оружие': 'weapon', 'броня': 'armor', 'шлем': 'helmet', 'щит': 'shield', 'аксессуар': 'accessory', 'аксессуар2': 'accessory2',
    'weapon': 'weapon', 'armor': 'armor', 'helmet': 'helmet', 'shield': 'shield', 'accessory': 'accessory', 'accessory2': 'accessory2'
  };

  if (slotMap[slotOrName.toLowerCase()]) {
    slotToUnequip = slotMap[slotOrName.toLowerCase()];
  } else {
    // Try to find by item id or name
    for (const [slot, itemId] of Object.entries(rpg.equipment)) {
      if (itemId === slotOrName || (itemId && getItem(itemId)?.name.toLowerCase() === slotOrName.toLowerCase())) {
  ...
  (rpg.equipment as any)[slotToUnequip] = null;
