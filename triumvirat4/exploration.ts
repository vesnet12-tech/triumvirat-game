...
export function getExplorationKeyboard(char: any) {
  ...
  kb.textButton({ label: (char.rpg?.activeQuest ? '🔴 Мое задание' : '📋 Задания'), payload: { command: 'guild_my_quest', from: 'explore' }, color: (char.rpg?.activeQuest ? Keyboard.NEGATIVE_COLOR : Keyboard.SECONDARY_COLOR) });
  kb.row();
