...
for (const key in catalog) {
    ...
    if (type !== 'armor' && type !== 'helmet' && type !== 'accessory' && !key.startsWith('arm_new_') && !key.startsWith('acc_new_') && !key.startsWith('arm_gen_') && !key.startsWith('acc_gen_') && !key.startsWith('arm_req_') && !key.startsWith('acc_req_') && key !== 'boss_ent_acc' && key !== 'boss_ent_acc_leg') {
...
const ACC_ARCHETYPES = [
  { id: 'crit', n: 'Осколок Звезды', d: 'Усиливает атаки.', stats: (m: number, t: number) => ({ attack: 3*m, agility: 3*m, critRate: (t >= 4 ? getPercent(t, 1.5) : 0) }) },
  { id: 'crit_dmg', n: 'Сапфир Ярости', d: 'Наполняет удары злобой.', stats: (m: number, t: number) => ({ attack: 4*m, maxHp: 10*m, critDamage: (t >= 4 ? getPercent(t, 4) : 0) }) },
  ...
  { id: 'berserk', n: 'Кольцо Ярости', d: 'Значительно повышает урон, понижая защиту.', stats: (m: number, t: number) => ({ attack: 8*m, critRate: (t >= 4 ? getPercent(t, 1.5) : 0), defense: -4*m, magicDefense: -4*m }) },
  { id: 'sniper', n: 'Глаз Сокола', d: 'Фокусирует зрение на уязвимых точках.', stats: (m: number, t: number) => ({ agility: 5*m, attack: 3*m, critDamage: (t >= 4 ? getPercent(t, 3) : 0) }) },
  ...
  { id: 'hunter_acc', n: 'Клык Хищника', d: 'Острый, как бритва.', stats: (m: number, t: number) => ({ attack: 5*m, agility: 2*m, critRate: (t >= 4 ? getPercent(t, 1) : 0) }) },
  ...
  { id: 'space_acc', n: 'Космическая Пыль', d: 'Частица далекой звезды.', stats: (m: number, t: number) => ({ magicAttack: 6*m, maxMp: 10*m, critDamage: (t >= 4 ? getPercent(t, 2) : 0) }) },
...
ARMOR_ARCHETYPES.forEach(arch => {
  RARITY_TIERS.forEach(tier => {
    ...
    if (tier.tier === 5) {
       let buffIndex = Math.floor(Math.random() * LEGENDARY_BUFFS.length);
       if (buffIndex === 0) {
           finalStats.attack = (finalStats.attack || 0) + 15;
           finalStats.defense = (finalStats.defense || 0) + 15;
           finalStats.magicAttack = (finalStats.magicAttack || 0) + 15;
       ...
       suffix = LEGENDARY_BUFFS[buffIndex].desc;
    }
